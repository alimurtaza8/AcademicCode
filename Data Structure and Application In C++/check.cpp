...
// LinkedList structure for storing voter information
struct VoterNode {
    int data;
    bool hasVoted;
    VoterNode* next;
...
// Functions to manage the linked list of voters
...
void printVoters() {
    VoterNode* ptr = head;
    while (ptr != NULL) {
        cout << ptr->data << (ptr->hasVoted ? " (voted)" : "") << ", ";
        ptr = ptr->next;
    }
    cout << endl;
}
...
Candidate candidates[100];  // Maximum number of candidates
int candidateCount = 0;

// Functions to manage voting
...
// Binary Search Tree for fast candidate lookup
...
CandidateBSTNode* insert(CandidateBSTNode* root, string data) {
    if (root == NULL) {
        ...
        root->left = NULL;
        root->right = NULL;
        addCandidate(data);  // Add to the candidate array as well
        return root;
    }
